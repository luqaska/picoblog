    ...
    public function renderEntries(array $entries, $show, string $entryWrap = '<li>{entry}</li>', bool $parseTags = false)
    {
        ...
        foreach ($entries as $id => $entry) {
            ...
            if($show!=true){
              ...
              $text = "<meta property='og:title' content='$id' /><div class='e show'><h3><a href='?'><img class='pp' src='{$avatar}'> {$nick}</a> • <span class='date' title='{$date}'>{$date_p}</span><span class='date' style='float:right'>#{$id}<span></h3><h2 class='text'>" . $text . "</h2></div>{$ut}";
            }
